...
export const tsCloud: TsCloudConfig = {
  ...
  infrastructure: {
    ...
    compute: {
      ...
      // Install PostgreSQL on the box (Pantry) — bughq stores errors/issues in
      // Postgres, and there's no external managed cluster, so it lives on the
      // same server (localhost:5432). The deploy creates the `bughq` database.
      managedServices: { postgres: true },
      database: { engine: 'postgres', name: 'bughq', username: 'bughq', password: env.DB_PASSWORD || 'bughq_prod_pw' },
      webServer: 'rpx',
      proxy: {
        engine: 'rpx',
        onDemandTls: true,
    ...
    ssl: {
      ...
      domains: env.SSL_DOMAINS?.split(',') || ['bughq.org', 'www.bughq.org'],
      redirectHttp: true,
      // Porkbun manages DNS for bughq.org. ts-cloud auto-detects the provider
      // from the domain's nameservers and reads PORKBUN_API_KEY / PORKBUN_SECRET_KEY
      // from the environment (.env) to create ACM validation + apex/CNAME records.
     ...
     */
    dns: {
      // bughq.org is registered + DNS-managed at Porkbun (provider auto-detected
      // by ts-cloud from the domain's nameservers + PORKBUN_* creds above).
   ...
   */
  sites: {
    // The bughq app — a Bun server (`buddy serve`) rendering the stx issues
    ...
    // www → apex redirect.
    www: { domain: `www.${env.APP_DOMAIN || 'bughq.org'}`, redirect: `https://${env.APP_DOMAIN || 'bughq.org'}` },
  },
}
